...
$checks = @(
  ...
  @{
    ...
    Run  = {
      foreach ($t in "model", "loader", "pivot", "presets", "validator", "exporter", "select", "vectorize", "segment", "segment-quality", "path-bbox", "layer-ingest", "emit", "grade") {
  ...
  @{
    ...
    Run  = {
      foreach ($t in "tools", "server", "protocol", "vectorize-vtracer", "regions-preview", "smiley-golden") {
