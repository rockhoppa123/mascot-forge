...
| Q7 | v1.1 generalisation: which DevBrain-tuned heuristics/strings need data-driving so a new asset needs no engine edit? | 🟡 | Surfaced by Spike 03 (second asset ran with **zero engine edits**, but several DevBrain hard-codes were *worked around*). Backlog, priority order: (1) **data-drive the P2 part vocabulary** (`segment-parts.ps1` is tuned to body/legs/antenna/eyes/moustache — the real wedge); (2) derive React `PART_IDS` from `rig.parts` (`emit.ts`); (3) carry asset identity through tools (flat viewBox + source path in segmenter; output basename + demo label in both emitters; drop the `OutDir` `devbrain-*`-only assertion); (4) address scale (O(W·H) vectorizer + O(n²) CCL — cap/declare source resolution or replace the union); (5) document the Clean-Mascot-Source contract (transparent PNG). See `spikes/03-second-asset/FINDINGS.md`. |
