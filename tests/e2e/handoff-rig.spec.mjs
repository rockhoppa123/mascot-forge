...
const URL = "/tools/rig-editor/index.html"; // ponytail: local name shadows global URL — use import.meta.dirname below, not new URL()
...
// forge_open_editor's `open` URL carries ?rig=<path>; the editor must auto-fetch + load it on startup
test("?rig= auto-loads a handoff rig animated", async ({ page }) => {
  const dir = join(import.meta.dirname, "fixtures");
  mkdirSync(dir, { recursive: true });
  writeFileSync(join(dir, "handoff-fixture.svg"), RIG_SVG);
  await page.goto(URL + "?rig=tests/e2e/fixtures/handoff-fixture.svg");
  await expect(page.locator("#rigstatus")).toContainText("idle ✓");
  const p = await page.evaluate(() => window.__rigEditor.model.preset("idle", "part-body"));
  expect(p).toBe("breathe");
});
